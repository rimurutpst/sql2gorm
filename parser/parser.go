...
func makeCode(stmt *ast.CreateTableStmt, opt options) (string, []string, error) {
	...
	for _, col := range stmt.Cols {
		...
		if colName == "created_at" {
			gormTag.WriteString(";autoCreateTime:milli")
		}
		if colName == "updated_at" {
			gormTag.WriteString(";autoUpdateTime:milli")
		}
		if colName == "deleted_at" {
			gormTag.WriteString(";softDelete:milli")
		}
		tags = append(tags, "gorm", gormTag.String())

		if opt.JsonTag {
			tags = append(tags, "json", underscore2CamelCase(colName))
		}
		...
		if colName == "deleted_at" {
			field.GoType = "soft_delete.DeletedAt"
		} else {
			field.GoType = goType
		}
...
func underscore2CamelCase(field string) string {
